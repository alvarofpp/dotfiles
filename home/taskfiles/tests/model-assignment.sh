...
caso() {
  local obtido
  obtido=$(jq -nc --argjson disp "$2" --argjson hist "$3" --argjson planejado "$4" "$JQM" \
...
caso "tudo com chave, sem histórico" "$todos" '[]' false \
  "opus deepseek-v4-pro minimax-m3 opus deepseek-v4-flash"
...
caso "revisão fora da família da execução" '["opus","haiku","glm-5.3","glm-5.3-flash","kimi-k3"]' \
  ...
  "opus glm-5.3 glm-5.3-flash opus haiku"
...
caso "issue já planejada" "$todos" '[]' true \
  "null null minimax-m3 opus deepseek-v4-flash"
