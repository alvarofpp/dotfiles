...
- **systemd-user**: `home/.config/systemd/user/gh-board-defaults.{service,timer}` roda a cada 15 min o tick do GitHub Project — `gate-watch`, `sync`, `reap`, `unblock`, `answer`, `prep`, `dispatch`, `pr-tick`, `review-requested`, `unstick`, `automerge`, `settle`, `close`, `pr-board`, `done`, `notify` e `merge-policy`, nessa ordem — a ordem é a semântica, não arrumação. O `merge-policy` fecha a fila e se autolimita a uma passada por dia. Units do submódulo `ai/` (Hermes, opencode, svc-ctl, orca-serve) ficam em `ai/.config/systemd/user/`.
