...
- **Taskfiles** (`home/Taskfile.yml` + `home/taskfiles/`) provide global tasks via `task` (aliased as `t`). Namespaces: `cc:` (Claude Code), `docker:`, `op:`, `py:`, `ai:` (Hermes services + plugin claude_code — definido em `ai/taskfiles/AI.yml`, incluído via path absoluto `$HOME/dotfiles/...` pra resolver o symlink do stow).
