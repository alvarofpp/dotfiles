...
- **O namespace `gh:` é o fluxo autônomo de review** — opera o GitHub Project "Agentes" (`alvarofpp/projects/3`); `task --list-all | grep '^\* gh:'` lista as tasks. O Status da issue **é** a máquina de estados: cada tick lê uma coluna, tria (`prep`), implementa (`dispatch`), revisa (`pr-tick`), destrava (`unblock`, `answer`, `unstick`), confere (`settle`) e fecha (`close`). O **merge é o único passo humano**, junto de `Backlog → Ready` e `Dropped` — exceto no que o `automerge` liberar. O interruptor dele é `~/.local/state/gh-board/automerge` (um repo por linha; `cat` nele diz quem está autorizado, arquivo ausente desliga geral), ligado desde 2026-09-12. A referência completa — os oito Status, a ordem exata do tick, o vocabulário (labels `agent:*`, marcadores, `impl:`/`review:`/`agent:`), sub-issues, os portões de segurança, os filtros das views e os modos de falha conhecidos — está em **[`ai/docs/gh-board-flow.md`](ai/docs/gh-board-flow.md)**; não duplique aqui.
