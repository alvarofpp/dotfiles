...
- **Taskfiles** (`home/Taskfile.yml` + `home/taskfiles/`) provide global tasks via `task` (aliased as `t`). Namespaces: `cc:` (Claude Code), `docker:`, `op:`, `py:`, `ai:` (Hermes services + plugin claude_code + opencode standalone — definido em `ai/taskfiles/AI.yml`, incluído via path absoluto `$HOME/dotfiles/...` pra resolver o symlink do stow). `ai:` inclui `restart-opencode`, `logs-opencode`, `opencode-sync-agent` além das tasks de Hermes.
