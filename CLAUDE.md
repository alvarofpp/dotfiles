...
Este repo **não tem** `TODO.md` nem `CHANGELOG.md` — deixaram de existir em 2026-09-05, aqui e em todos os repos do board, porque eram a fonte nº 1 de conflito entre PRs de agentes paralelos.
...
- **O namespace `gh:` é o fluxo autônomo de review** — opera o GitHub Project "Agentes" (`alvarofpp/projects/3`); `task --list-all | grep '^\* gh:'` lista as tasks. O Status da issue **é** a máquina de estados: cada tick lê uma coluna, tria (`prep`), implementa (`dispatch`), revisa (`pr-tick`), destrava (`unblock`, `answer`, `unstick`), confere (`settle`) e fecha (`close`). O **merge é o único passo humano**, junto de `Backlog → Ready` e `Dropped` — exceto no que o `automerge` liberar. O interruptor dele é `~/.local/state/gh-board/automerge` (um repo por linha, `!` nega e ganha de qualquer autorização; `cat` nele diz quem está autorizado, arquivo ausente desliga geral), ligado desde 2026-09-12. A referência completa — os oito Status, a ordem exata do tick, o vocabulário (labels `agent:*`, marcadores, `impl:`/`review:`/`agent:`), sub-issues, os portões de segurança, os filtros das views e os modos de falha conhecidos — está em **[`ai/docs/gh-board-flow.md`](ai/docs/gh-board-flow.md)**; não duplique aqui.
- **systemd-user**: `home/.config/systemd/user/gh-board-defaults.{service,timer}` roda a cada 10 min o tick do GitHub Project — `gate-watch`, `sync`, `reap`, `sweep`, `unblock`, `answer`, `prep`, `dispatch`, `pr-tick`, `review-requested`, `unstick`, `automerge`, `settle`, `close`, `pr-board`, `done`, `metrics`, `merge-policy` e `notify`, nessa ordem — a ordem é a semântica, não arrumação. O `merge-policy` se autolimita a uma passada por dia, e o `notify` fecha a fila. Units do submódulo `ai/` (Hermes, opencode, svc-ctl, orca-serve, rendeira e rendeira-backup) ficam em `ai/.config/systemd/user/`.
- **`docker-gc.{service,timer}`** roda `task docker:gc` às 05:00 — build cache, imagem pendurada e volume **anônimo**. Não toca volume nomeado: nomeado é banco de instância, e instância parada com `dev:down` não é instância morta. Volume de worktree que sumiu sai pelo `task docker:gc:instancias`, que é manual e pede confirmação.
